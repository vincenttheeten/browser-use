(
    ...
    const interactiveNonContainerElements = new Set([
        'button', 'embed', 'input', 'menuitem',
        'object', 'select', 'textarea'
    ]);

    const interactiveNonContainerRoles = new Set([
        'button', 'checkbox', 'radio', 'slider',
        'switch', 'textbox', 'progressbar', 'scrollbar',
        'spinbutton', 'searchbox', 'tooltip'
    ]);

    let highlightIndex = 0; // Reset highlight index

    function highlightElement(element, index, parentIframe = null) {
        console.log("Highlighting element", element, index);
    ...
    function isInteractiveElement(element, includeClickStyling = false) {
        ...
        const hasClickStyling =
            style.cursor === 'pointer' || // Explicitly has a pointer cursor
            element.style.cursor === 'pointer'; // Inline style has pointer cursor
        ...
        return hasAriaProps ||
            (includeClickStyling && hasClickStyling) ||
            hasClickHandler ||
            hasClickListeners ||
            // isFormRelated ||
            isDraggable;
    ...
    function shouldRecurseIntoElement(element) {
        if (!(element instanceof HTMLElement)) {
            return true;
        }

        const tagName = element.tagName ? element.tagName.toLowerCase() : null;
        if (tagName && interactiveNonContainerElements.has(tagName)) {
            return false;
        }
        
        const role = element.getAttribute('role');
        if (role && interactiveNonContainerRoles.has(role)) {
            return false;
        }
        
        const ariaRole = element.getAttribute('aria-role');
        if (ariaRole && interactiveNonContainerRoles.has(ariaRole)) {
            return false;
        }
    ...
    function buildDomTree(node, parentIframe = null) {
        ...
        if (node.nodeType === Node.ELEMENT_NODE) {
            const isInteractiveWithStyling = isInteractiveElement(node, true);
            const isInteractiveStrict = isInteractiveWithStyling ? isInteractiveElement(node, false) : false;
            const isInteractive = isInteractiveStrict || isInteractiveWithStyling;
            const isVisible = isElementVisible(node);
            const isTop = isTopElement(node);

            nodeData.isInteractive = isInteractive;
            nodeData.isInteractiveStrict = isInteractiveStrict;
            nodeData.isInteractiveWithStyling = isInteractiveWithStyling;
            nodeData.isVisible = isVisible;
            nodeData.isTopElement = isTop;

            // Add element coordinates in the viewport, 
            // so we can also click on the element by their position
            const rect = node.getBoundingClientRect();
            nodeData.centerX = Math.round(rect.left + rect.width / 2);
            nodeData.centerY = Math.round(rect.top + rect.height / 2);

            if (parentIframe) {
                const iframeRect = parentIframe.getBoundingClientRect();
                nodeData.centerX += iframeRect.left;
                nodeData.centerY += iframeRect.top;
            }
        ...
        if (node.tagName === 'IFRAME') {
            ...
            if (!shouldRecurseIntoElement(node)) {
                console.log("Node cannot contain other interactive elements, skipping further recursion", node);
                console.log(nodeData);
            }
            else if (!nodeData.isInteractiveStrict && nodeData.isInteractiveWithStyling) {
                console.log("Node was made interactive with styling but not strict... skipping further recursion", node);
                console.log(nodeData);
            } else {
